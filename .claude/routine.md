...
This file documents the project-specific commands, conventions, and quirks for **test-repo-only.git**. It supplements `routine-base.md` (which has the universal discipline) with project-specific details Claude runs need to know.
...
- **Source code:** ``
- **Tests:** `tests/`
- **Build output:** `n/a` (typically `dist/` or `build/`)

When the user references a file by bare name (e.g. `main.js`), it lives in `/`. When they reference a test file (e.g. `main.test.js`), it lives in `tests//`.
...
**Deploy target:** none (backlog/storage repo)
...
>
> - **Data layer routing:** "All data-model mutations must go through `/dataLayer.js`. Direct localStorage writes are prohibited."
...
> - **Known fragile files:** "/legacy.js is in active rewrite — changes there require extra care."
...
>
> - "/main.js evaluates module-level code twice due to webpack entry-bundle structure. Use one-shot guards for listener registration."
...
**Variant:** (none — repo-only) (either `gen-src-manifest.js` for CommonJS projects or `gen-src-manifest.cjs` for ESM projects with `"type": "module"` in package.json)
...
>
> - "test-repo-only.git is a single-page app; routing is hash-based via {{ROUTER_FILE}}."
> - "The main state container is {{STATE_FILE}} — global state mutations flow through there."
> - "UI components live in /components/; pages in /pages/."
>
...
>
> - "Run `none` before opening the PR — CI will catch failures but the agent should not rely on CI to find them."
